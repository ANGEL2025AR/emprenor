...
async function getPunchLists(user: SerializableUser) {
  try {
    ...
    return lists.map((list) => ({
      ...
      inspectionId: list.inspectionId?.toString() || undefined,
      phase: list.phase || "",
      location: list.location || "",
      createdBy: list.createdBy?.toString() || "",
      status: list.status || "abierta",
      items: list.items || [],
...
export default async function PunchListsPage() {
  ...
  const lists = await getPunchLists(user)
