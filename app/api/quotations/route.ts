...
export async function GET() {
  try {
    ...
    const serializedQuotations = quotations.map((quotation) => ({
      ...
      createdAt: quotation.createdAt ? new Date(quotation.createdAt).toISOString() : new Date().toISOString(),
      updatedAt: quotation.updatedAt ? new Date(quotation.updatedAt).toISOString() : new Date().toISOString(),
      validUntil: quotation.validUntil ? new Date(quotation.validUntil).toISOString() : null,
      approvedAt: quotation.approvedAt ? new Date(quotation.approvedAt).toISOString() : null,
